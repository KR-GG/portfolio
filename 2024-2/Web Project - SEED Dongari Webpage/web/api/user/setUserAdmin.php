...
if ($user_role !== Role::ADMIN->value) {
    ...
    echo json_encode(["status" => "error", "message" => "Not authorized to set user as admin"]);
    exit();
}
...
if(isset($data['user_id'])) {
    ...
    if ($user['role'] === Role::USER->value) {
        $result = $userService->changeUserRole($user_id, Role::ADMIN->value);
        ...
        // Only users can be set as admin
        http_response_code(400);
        echo json_encode(["status" => "error", "message" => "Only users can be set as admin"]);
        exit();
    }
} else {
